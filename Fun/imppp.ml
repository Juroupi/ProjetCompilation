...
let rec pp_expression i lp rp =
  match i with
  | Cst(n) -> string_of_int n
  | Bool(b) -> if b then "true" else "false"
  | Var(x) -> x
  | Unop(op, e) ->
    sprintf "%s%s%s%s" lp (pp_unop op) (pp_expression e "(" ")") rp
  | Binop(op, e1, e2) ->
    sprintf "%s%s %s %s%s" lp (pp_expression e1 "(" ")") (pp_binop op) (pp_expression e2 "(" ")") rp
  | Call(f, args) ->
    sprintf "%s(%s)" f (pp_args args)
  | Deref(Binop(Add, e1, Binop(Mul, e2, Cst 4))) ->
    sprintf "%s[%s]" (pp_expression e1 "(" ")") (pp_expression e2 "" "")
  | Deref(e) ->
    sprintf "%s*%s%s" lp (pp_expression e "(" ")") rp
  | Addr(x) ->
    sprintf "%s&%s%s" lp x rp
  | PCall(f, args) ->
