...
let iter_edges f g =
  VMap.iter (fun x v -> VMap.iter (fun y t -> f x y t) v) g

exception Edge_found of (string * string * tag) ;;

let find_first_edge_opt f g =
  try
    iter_edges (fun x y t -> 
      if f x y t then raise (Edge_found (x, y, t))
    ) g;
    None
  with
    | Edge_found e -> Some e